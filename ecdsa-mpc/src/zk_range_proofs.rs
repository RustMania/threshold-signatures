...
// unexpected warning here, can be a bug in the compiler
#[allow(unused_imports)]
use algorithms::config::{DEFAULT_GROUP_ORDER_BIT_LENGTH, DEFAULT_SAFE_PRIME_BIT_LENGTH};

use algorithms::types::Powm;
use algorithms::types::{BasicOps, BitManipulation, Integer, Modulo, One, Samplable, Zero};
use std::borrow::Borrow;
use zeroize::Zeroize;

use crate::types::{CurvDLogProofType, DLogProof};
use algorithms::dlog_proof::DlogProof;
use algorithms::nizk_rsa;
use algorithms::primes::PairOfSafePrimes;
use algorithms::sha::HSha512Trunc256;
...
fn pair_of_safe_primes(bit_length: usize) -> PairOfSafePrimes {
    ...
    let (p, p_prim) = primes::random_safe_prime(bit_length);
    let (q, q_prim) = primes::random_safe_prime(bit_length);
    PairOfSafePrimes {
        p,
        p_prim,
        q,
...
impl ZkpSetup {
    ...
    pub fn random(group_order_bit_length: usize) -> Self {
        use algorithms::utils::sample_generator_of_rsa_group;
        ...
        let alpha = loop {
            let alpha = BigInt::strict_sample_range(&BigInt::one(), &(phi.borrow() / 4));
...
impl Zeroize for AliceZkpInit {
    fn zeroize(&mut self) {
        self.alice_pk.n.zeroize();
        self.alice_pk.nn.zeroize();
...
impl AliceProof {
    ...
    pub fn verify(
        ...
        if self.s1 > Scalar::group_order().pow(3) {
            log::trace!("proof.s1 is larger than q^3");
            return false;
        }

        let z_e_inv = {
            let x = self.z.powm_sec(&self.e.0, N_tilde);
            BigInt::mod_inv(&x, N_tilde)
        };
        if z_e_inv.is_none() {
        ...
        let cipher_e_inv = {
            let x = cipher.powm_sec(&self.e.0, NN);
            BigInt::mod_inv(&x, NN)
        };
...
impl MessageB {
    ...
    pub fn new(
        ...
        let beta_prim_fe = Scalar::from_bigint(&beta_prim);
        let beta = Scalar::zero() - &beta_prim_fe;
...
impl Zeroize for BobZkpInit {
    fn zeroize(&mut self) {
        self.alice_ek.n.zeroize();
        self.alice_ek.nn.zeroize();
...
impl BobZkpRound1 {
    ...
    fn from(init: &BobZkpInit, b: &Scalar, beta_prim: &BigInt, a_encrypted: &BigInt) -> Self {
        let b_bn = b.to_bigint();
        Self {
...
impl BobZkpRound2 {
    ...
    fn from(init: &BobZkpInit, e: &BigInt, b: &Scalar, beta_prim: &BigInt, r: &Randomness) -> Self {
        let b_bn = b.to_bigint();
        Self {
...
impl BobProofExt {
    pub fn verify(
        ...
        let e = HSha512Trunc256::create_hash_with_nonce(
            &[
                &alice_ek.n,
                &Gen,
                &self.X.x_coord().unwrap(),
                &self.X.y_coord().unwrap(),
                a_enc,
                mta_avc_out,
                &self.u.x_coord().unwrap(),
                &self.u.y_coord().unwrap(),
                &self.proof.z,
                &self.proof.z_prim,
                &self.proof.t,
                &self.proof.v,
        ...
        let (x1, x2) = {
            ...
            let s1 = Scalar::from_bigint(&self.proof.s1);
            let e = Scalar::from_bigint(&self.proof.e.0);
    ...
    pub fn generate(
        ...
        let (X, u) = {
            let ec_gen = Point::generator();
            let alpha = Scalar::from(&init.alpha);
...
mod tests {
    ...
    use super::*;
    use crate::types::PaillierKeys;
    use algorithms::primes::is_prime;

    #[test]
    fn hash_algorithm_compliant() {
        assert!(HSha512Trunc256::can_handle_curve_modulo(
            &Scalar::group_order()
        ))
    }
            ...
            (0..5).for_each(|_| {
                ...
                match msgb.proof {
                    BobProofType::RangeProofExt(proof) => {
                        ...
                        let alpha = Scalar::from(&alice_share);
    ...
    #[test]
    fn check_bitness() {
        use algorithms::dlog_proof::DIGEST_BIT_LENGTH;
