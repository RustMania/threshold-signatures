...
use crate::types::{BasicOps, BitManipulation, Converter};
...
mod tests {
    ...
    fn hash_with_random_nonce() {
        (1..1000).for_each(|_| {
            let alpha = BigInt::sample_below(&Scalar::group_order().pow(3));
            let beta = BigInt::sample_below(&Scalar::group_order().pow(3));
